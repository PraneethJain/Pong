...
class Slider:
    ...
    def __init__(self, width: int = 250, pos: tuple[int, int] = (SCREEN_WIDTH/2, SCREEN_HEIGHT/2)):
        self.image = pg.Surface((width, 5))
        self.image.fill((100, 100, 100))
        self.rect = self.image.get_rect(center=pos)
        self.current = 100
        self.filled = pg.Surface((self.current*self.image.get_width()/100, self.image.get_height()))
    ...
    def draw(self):
        ...
        pg.draw.circle(screen, (0, 128, 128), (self.rect.left+self.current/100*self.rect.width, self.rect.centery),8)
